...
if(isset($_POST['tambah'])){
    ...
    if(mysqli_num_rows($cek)==0){
        $insert = mysqli_query($con,"INSERT INTO users (nama, departemen, username, password, level) VALUES ('$nama','$departemen','$username','$password','$level')") or die (mysqli_error($con));
        if($insert){
...
if(isset($_POST['ubah'])){
    ...
    if($password!=""){
        $update = mysqli_query($con,"UPDATE users SET nama='$nama', departemen='$departemen', password='".password_hash($password,PASSWORD_DEFAULT)."' WHERE id_users='$id'") or die (mysqli_error($con));
    }else{
        $update = mysqli_query($con,"UPDATE users SET nama='$nama', departemen='$departemen' WHERE id_users='$id'") or die (mysqli_error($con));
    }
    if($update){
